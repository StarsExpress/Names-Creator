...
surname_creator, forename_creator = SurnameCreator(), ForenameCreator()


def make_creations(number, lock, preference='remix'):  # Create names upon called by Streamlit app.
    start = time.time()  # Start time of creation.
    surnames_list, forenames_list = [], []  # Initialize two empty lists.
    ...
    if preference != 'just_surname':  # If not just surname, forenames must be needed.
        forenames_list = forename_creator.create(number, lock)
