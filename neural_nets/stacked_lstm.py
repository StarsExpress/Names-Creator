...
def make_stacked_lstm(input_shape, metrics_list):  # Input shape is a tuple of (timesteps, features).
    ...
    model.add(LSTM(NEURONS_UNITS[0], activation=ACTIVATIONS[0], input_shape=input_shape, return_sequences=True,
                   dropout=DROPOUT_RATE, recurrent_dropout=DROPOUT_RATE))

    # 2nd masking layer.
    model.add(Masking(mask_value=PAD_VALUE))
    # 2nd LSTM layer.
    model.add(LSTM(NEURONS_UNITS[1], activation=ACTIVATIONS[1], dropout=DROPOUT_RATE, recurrent_dropout=DROPOUT_RATE))
